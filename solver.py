...
def main():
	...
	all_data = pd.read_csv('../sna-java/output/linearEqs.csv')
	print(all_data)
	print('\n')

	#df = pd.DataFrame(columns=['p-','q-','p+','q+'])
	#df = pd.DataFrame(columns=['GraphNo', 'p-','q-','p+','q+'])


	#for graphNo in range(1,501):
	...
	#df = df.append({'p-':pMinus,'q-':qMinus,'p+':pPlus,'q+':qPlus}, ignore_index=True)
	
	#df.to_csv('output/bernoulliSimpleAnalysis.csv', sep=',')
	print('Finished')
